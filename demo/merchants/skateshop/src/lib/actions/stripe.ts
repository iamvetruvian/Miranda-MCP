...
export async function getStripeAccount(
  ...
  try {
    ...
    if (!payment || !payment.stripeAccountId || payment.stripeAccountId.startsWith("acct_demo")) {
      if (store && process.env.STRIPE_API_KEY) {
        return {
          isConnected: true,
          account: null,
          payment: {
            stripeAccountId: store.stripeAccountId || "platform",
            detailsSubmitted: true,
          } as any,
        }
      }
    ...
    try {
      account = await stripe.accounts.retrieve(payment.stripeAccountId)
    } catch {
      if (store && process.env.STRIPE_API_KEY) {
        return {
          isConnected: true,
...
export async function createPaymentIntent(
  ...
  try {
    ...
    const isConnect =
      ...
      !payment.stripeAccountId.startsWith("acct_demo") &&
    ...
    const paymentIntent = await stripe.paymentIntents.create(
      {
        amount: total,
        ...(isConnect ? { application_fee_amount: fee } : {}),
        currency: "usd",
        metadata,
      ...
      isConnect ? { stripeAccount: payment.stripeAccountId } : undefined
    )
