...
export async function getOrderLineItems(
  ...
  try {
    let parsedInput: unknown = input.items
    ...
    if (!safeParsedItems.success) {
      console.error("Could not parse items in getOrderLineItems:", safeParsedItems.error)
      return []
    }

    const productIds = safeParsedItems.data.map((item) => item.productId)
    const dbProducts =
      productIds.length > 0
        ? await db
            .select({
    ...
    const lineItems: CartLineItemSchema[] = safeParsedItems.data.map((item) => {
      ...
      return {
        ...
        price: dbProduct?.price ?? Number(item.price).toFixed(2),
        inventory: dbProduct?.inventory ?? 0,
        quantity: item.quantity ?? 1,
        storeId: dbProduct?.storeId ?? input.storeId ?? "",
      }
    })
